...
function audiodevwatch(dev_uid, event_name, event_scope, event_element)
   ...
   if event_name == 'jack' then
      if dev:jackConnected() then
         ...
         if mod.config.control_hermes and hermes_was_playing then
            logger.d("Resuming playback in Hermes")
            notify("Headphones plugged", "Resuming Hermes playback")
            hermes.play()
         end
      else
         ...
         hermes_was_playing  = hermes.isPlaying()
         logger.df("hermes_was_playing=%s", hermes_was_playing)
         if mod.config.control_spotify and spotify_was_playing then
         ...
         if mod.config.control_hermes and hermes_was_playing then
            logger.d("Pausing Hermes")
            notify("Headphones unplugged", "Pausing Hermes")
            hermes.pause()
         end
      end
   end
